...
public sealed class EmailService : IEmailService
{
    ...
    private static readonly Lazy<byte[]> _logoBytes = new(() =>
    {
        ...
        using var stream = asm.GetManifestResourceStream("BirthChain.Infrastructure.Resources.logo.png")
                           ?? throw new FileNotFoundException("Embedded logo resource not found.");
    ...
    private const string LogoCid = "birthchain-logo";
    ...
    private static string Wrap(string innerHtml) => $"""
        <div style="font-family:'Segoe UI',Arial,sans-serif;max-width:520px;margin:0 auto;padding:0;border:1px solid #e0e0e0;border-radius:12px;overflow:hidden;">
            <div style="background:#1A3C6D;padding:20px;text-align:center;">
                <img src="cid:{LogoCid}" alt="BirthChain" style="height:56px;width:auto;display:inline-block;margin-bottom:8px;" />
            ...
            <div style="background:#f5f5f5;padding:14px 28px;text-align:center;font-size:11px;color:#999;">
                <img src="cid:{LogoCid}" alt="" style="height:20px;width:auto;vertical-align:middle;margin-right:6px;opacity:0.6;" />
    ...
    private async Task SendAsync(string toEmail, string subject, string htmlBody)
    {
        ...
        // Build an AlternateView with inline logo attachment (CID)
        var htmlView = AlternateView.CreateAlternateViewFromString(htmlBody, null, MediaTypeNames.Text.Html);

        var logoStream = new MemoryStream(_logoBytes.Value);
        var logoResource = new LinkedResource(logoStream, MediaTypeNames.Image.Png)
        {
            ContentId = LogoCid,
    ...
    {
        // Try to extract meaningful fields from JSON description
    ...
    public async Task SendPasswordResetConfirmationAsync(string toEmail, string fullName)
    {
        var inner = $"""
            ...
            <div style="background:#FFF8F0;border-left:4px solid #F58B1F;padding:12px 16px;margin:16px 0;border-radius:0 8px 8px 0;">
                <strong style="color:#F58B1F;">⚠️ Didn't do this?</strong> If you did not reset your password, please contact support immediately as your account may be compromised.
            </div>
    ...
    private static string FormatRecordDescription(string description)
    {
        try
        {
            var doc = System.Text.Json.JsonSerializer.Deserialize<Dictionary<string, object>>(description);
            if (doc is not null)
            {
                var html = "";
